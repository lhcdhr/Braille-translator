...
def is_all_caps(s):
    ...
    False
    '''
    if s.isalpha()== True:  # to check if s is composed of letters only
        
        if len(s) >=2: # to check the length of s. Kick out all one-letter s.
            for char in s:      # use for loop to check if all char in s are capitalized. 
                if is_capitalized(char)== False:
                    return False    # when one of the char is not capitalized, return False.

            return True  # when the for loop is finished and there is no False returned, 
                         # it means that all char in s are capitalized. Return True.
        else:
            return False # Return False if the length of s is lower than 2

    else:
        return False     # Return False if there something other than letters.
...
def word_to_braille(word):
    ...
    '''
    braille_word=''  # store the converted characters
    
    two_rows = '\n\n' 

    if is_all_caps(word)==True:     # If the word is all capitalized, then the sign of capitalization need to be added 
                                    # in front of the word.     
        
        
        for char in word:
            
            braille_word = braille_word + char_to_braille(char) + two_rows # convert each character

        braille_word = CAPITAL + two_rows +  CAPITAL + two_rows + braille_word # add the sign to show it is all cap.

        return braille_word

    else:  # If it is not all cap, then check each cases and add their signs of representations.

        for char in word:
            
            if is_capitalized(char) == True:
                
                braille_word = braille_word + CAPITAL + two_rows + char_to_braille(char) + two_rows
                
            elif is_digit(char)== True:

                braille_word = braille_word + NUMBER + two_rows + char_to_braille(char) + two_rows
                
            else:
                
                braille_word = braille_word + char_to_braille(char) + two_rows
...
def new_filename(fname, addition):
    ...
    '''
    addition = "_"+addition+"."   # edit the addition with "_" and "."
    new_name = fname.replace(".",addition)  #replace the "." in fname with varaible addition to form the new file name
