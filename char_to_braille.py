...
def convert_irregular(c):
    ...
    position = IRREGULAR_CHARS.find(c)  # find the index of character
    if position == 0:                   # if index is 0, then it is space
        return SPACE
    elif position == 1 or position == 2:   # for index 1 and 2, they belongs to hyphen
        return HYPHEN
    elif position == 3 or position ==4 or position == 5:  #for 3,4, and 5, they belongs to apostrophe
        return APOSTROPHE
    else:              # the remaining two cases are for quotes
        return QUOTES
...
def convert_digit(c):
    ...
    ..
    '''
    position = DIGITS.find(c)      # get the index of c
    top = decade_pattern(position) # associate the index of c and its braille pattern
    return top + '\n..'            # return the converted braille
...
def convert_punctuation(c):
    ...
    position = PUNCTUATION.find(c)   #get the index of c in PUNCTUATION
    bottom = decade_pattern(position) #associate the 2 bottom rows for punctuation since row 0 is just '..'
    return "..\n"+bottom            # add '..' in row 0 and combine the following rows.
...
def decade_ending(dec_num):
    ...
    '.o'
    '''
    # decide the bottow row according to which decade this letter is in
    if dec_num==0:
        return '..'
    elif dec_num == 1:
        return 'o.'
...
def convert_letter(c):
    ...
    .o
    '''
    # combine the previous functions together
...
def char_to_braille(c):
    ...
    '\\n'
    '''
    # check one by one to see the type of character of c, and if it belongs to none of those types, return the same character
    if is_irregular(c) == True:
        return convert_irregular(c)
    elif is_digit(c) == True:
        return convert_digit(c)
    elif is_punctuation(c) == True:
        return convert_punctuation(c)
    elif is_letter(c) == True:
